...
async fn test_ft_total_supply() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let deposit_number: String = contract.view("get_deposit_number").await?.json()?;
    assert_eq!(deposit_number, "3");

    Ok(())
}

//TODO: falta hacer el test de integración de recive_near y pensar que es lo que tendría que cambiar en el contrato para que reciba tokens
